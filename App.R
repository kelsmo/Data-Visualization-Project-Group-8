...
MD <- read_csv("MDpop.csv") %>% select(c(1,4))
...
newcases<-totalcases[-1,1] %>% 
  ...
  rename(County=variable, NewCase=value,Date=DATE)

newdeaths <- totaldeaths[-1,1] %>% 
  cbind(data.frame(diff(as.matrix(totaldeaths[-1])))) %>% 
  gather(variable, value, -DATE)%>% 
  rename(County=variable, NewDeath=value, Date=DATE)
...
ui <- dashboardPagePlus(
    skin = "black",
    header = dashboardHeaderPlus(title = strong("COVID-19 & Stocks")),
    sidebar = dashboardSidebar(
        sidebarMenu(
            menuItem("About", tabName = "page5", icon=icon("home")),
            menuItem("COVID-19 Map", tabName = "page2", icon = icon("map-o")),
            menuItem("Trend in Maryland", tabName = "page1", icon = icon("line-chart")),
            menuItem("Cases vs. Stock Market", tabName = "page3", icon=icon("search-dollar")),
    ...
    body = dashboardBody(
        tabItems(
            ...
            tabItem(tabName = "page2",
                    column(3,
                           fluidRow(valueBoxOutput("TotalCases", width = "100%")),
                           fluidRow(valueBoxOutput("TotalDeaths", width = "100%")),
                           fluidRow(sliderInput("date2", label ="Date",
            ...
            tabItem(tabName = "page5",
                    fluidPage(
                      ...
                      tags$ul(
                        tags$li("For daily new confirmed cases and deaths in Maryland, we selected total confirmed cases and deaths from Feb 1st, 2020 to one day prior to the system date. Then we calculated the new confirmed cases and deaths by subtracting the i day from the i+1 day. Then we created dataset with the date, new confirmed cases, new deaths, and county."),
                      ...
                      h3("Methods and Analysis", align = "left",style="color:SteelBlue",tags$a(href="https://github.com/kclt/DataViz_Project/blob/master/Scripts/noaa_txt_download.R","[View Code]")),
                      strong("Map"),
                      ...
                      strong("Graphs are inspired by"),
                      tags$ul(
                      tags$li(a(href="http://freerangestats.info/blog/2016/08/18/dualaxes","Dual axes time series plots may be ok sometimes after all")),
                      tags$li(a(href="https://stackoverflow.com/questions/34370780/r-interactive-plot-show-vertical-line-on-hover","R interactive plot: show vertical line on hover")),
                      ),
                      strong("Data Source"),
                      tags$ul(
                      tags$li(a(href="https://github.com/CSSEGISandData/COVID-19/tree/master/csse_covid_19_data/csse_covid_19_time_series","Novel Coronavirus (COVID-19) Cases, provided by JHU CSSE")),
                      tags$li(a(href="https://github.com/nytimes/covid-19-data","Data on coronavirus cases and deaths in the U.S., provided by The New York Times")),
                      tags$li(a(href="https://coronavirus.maryland.gov/search?collection=Dataset","Maryland Department of Health")),
                      tags$li(a(href="https://finance.yahoo.com/","Yahoo Finance")),
                      tags$li(a(href="https://www.imf.org/en/Topics/imf-and-covid19/Policy-Responses-to-COVID-19#U","Policy Responses to COVID-19")),
                      tags$li(a(href="https://www.americanactionforum.org/insight/timeline-the-federal-reserve-responds-to-the-threat-of-coronavirus/","Timeline: The Federal Reserve Responds to the Threat of Coronavirus"))
                      )),
...
server <- function(input, output, session) {
  ...
  output$plot3 = renderPlotly({
    p3<-ggplot(aes(x=NewTest, y=NewCase, size= Pop,color=County),data=newtests %>% 
  ...
  g = reactive({
      ...
      g <- ggplot()  +  
        ...
        labs(title=paste0(input$stockmarkets,"During COVID-19"),
             caption = "Price.Close: Closing price from Yahoo Finance \nUs.New: New confirmed cases by day in the United States")
  ...
  output$TotalCases <- renderValueBox({
      valueBox(
        prettyNum(us$cases[which(us$date==input$date2)],big.mark = ","), "US Total Cases", icon = icon("head-side-mask"),
        color = "yellow"
      )
    })
  ...
  output$TotalDeaths <- renderValueBox({
      valueBox(
        prettyNum(us$deaths[which(us$date==input$date2)], big.mark=","), "US Total Deaths", icon = icon("viruses"),
        color = "aqua"
      )
    })
